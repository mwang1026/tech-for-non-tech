...
import { Chapter9DiagramSvg } from './Chapter9DiagramSvg'
import { CH9_FULL_VIEWBOX, CH9_REGION_VIEWBOX } from './chapter9Elements'
...
type Props = {
  /** Slide index within Chapter 9 — drives which elements are visible. */
  slideIndex: number
...
function regionForFocus(focus?: string): ViewBox {
  if (!focus) return CH9_FULL_VIEWBOX
  if (CH9_REGION_VIEWBOX[focus]) return CH9_REGION_VIEWBOX[focus]
  return CH9_FULL_VIEWBOX
}

export function Chapter9Diagram({ slideIndex, focus, highlight, highlightStatus }: Props) {
  const [vb, setVb] = useState<ViewBox>(CH9_FULL_VIEWBOX)
  ...
  const zoomBy = useCallback((factor: number) => {
    ...
    setVb(prev => {
      ...
      const w = clamp(prev.w / factor, CH9_FULL_VIEWBOX.w / 4, CH9_FULL_VIEWBOX.w * 2)
      const h = w * (CH9_FULL_VIEWBOX.h / CH9_FULL_VIEWBOX.w)
  ...
  const onSvgClick = useCallback((e: React.MouseEvent<SVGSVGElement>) => {
    ...
    if (!region) return
    const target_vb = CH9_REGION_VIEWBOX[region] ?? null
    if (!target_vb) return
    setManual(true)
    cancelTween.current?.()
      ...
      >
        ...
        >
          <motion.g animate={controls}>
            <Chapter9DiagramSvg slideIndex={slideIndex} highlight={highlight} highlightStatus={highlightStatus} />
          </motion.g>
        </svg>
