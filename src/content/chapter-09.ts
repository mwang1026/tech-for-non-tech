...
const theGap: Block[] = [
  ...
  p(_('This chapter is the journey from "merged into `main`" to "users see orange," plus what to do when orange turns out to be wrong.')),
]
...
const build: Block[] = [
  ...
  p(_('For the orange-sun change: the build runs from `main`, produces a fresh artifact containing the new `Hero.tsx` with "orange" in it, and wraps that artifact inside a container — the next slide.')),
  p(_('Build runs on a fresh machine, the same way CI does. The output is supposed to be reproducible: the same commit on `main` should produce the same artifact every time, so the artifact can be trusted as a faithful representation of the source.')),
]

/* --------------------------- Slide 3 — Containers --------------------------- */

const containers: Block[] = [
  p(_('A bug that ships every team eventually: the code runs on a developer\'s laptop, gets uploaded to a server, and crashes. The server\'s Linux is a slightly different version. The Node runtime is 20.3 instead of 20.5. A library installed locally isn\'t there. The phrase for this is "works on my machine."')),
  p(
    _('The fix is a '),
  ...
  p(
    _('The tool that defined the format and is still synonymous with containers in conversation is '),
    t('Docker', 'docker'),
    _(' (released 2013). When an engineer says "we run on Docker," they mean the back-end is packaged as Docker containers. Each container has a name and a tag — `myapp:42a1f2`, say, where the tag is the short hash of the commit it was built from. That tag is the breadcrumb back to the exact PR.'),
  ),
...
const registry: Block[] = [
  ...
  p(_('The reason this is its own step (instead of "build and deploy in one motion") is that the same image is going to be deployed to several places — dev, staging, production — at different moments. The registry is the shared shelf they all pull from.')),
]

/* --------------------------- Slide 5 — Environments --------------------------- */

const environments: Block[] = [
  p(_('The built image isn\'t shipped straight to real users. The standard setup runs the same image in three places, in increasing levels of "real."')),
  ul(
  ...
  p(_('Larger teams add more environments: a per-PR "preview" environment that builds an isolated copy of the app for the reviewer to click through (Vercel and Netlify do this automatically), a "QA" environment for the test team, a "performance" environment for load tests. Same rule applies: the closer to real users, the higher the bar to ship there.')),
]
...
const deployNaive: Block[] = [
  ...
  ul(
    ...
    [_('**Shipping becomes rare.** If every deploy is a visible interruption, the team ships less often. Less often means bigger changes per deploy, which means more risk per deploy, which is the reason teams used to ship quarterly with a pit in their stomachs.')],
  ),
  p(_('What we actually want: roll v2 out so v1 keeps serving users until v2 is healthy, with a way to undo instantly if something goes wrong. Three patterns do this.')),
]
...
const observability: Block[] = [
  ...
  p(
    _('Tools in this space: '),
    t('Sentry', 'sentry'), _(' for errors; '),
    t('Datadog', 'datadog'), _(', New Relic, and Honeycomb for metrics, dashboards, and log aggregation. Teams that start small lean on the cloud provider\'s built-in logging — '),
    t('AWS CloudWatch', 'cloudwatch'), _(' on AWS, '),
    t('GCP Cloud Logging', 'cloud-logging'), _(' on GCP — and add specialized tools as they grow.'),
  ),
  p(_('On the diagram, all three signals flow *out* of production — the arrows reverse direction relative to the deploy pipeline. Without observability, problems surface when a user tweets about them. With it, the team gets paged with the failing line of code already attached.')),
]
...
const rollback: Block[] = [
  ...
  p(_('When orange is fixed (better contrast, accessibility check passing, new tests added) the same pipeline runs again — new commit, new PR, new CI run, new build, new image, new canary, new observation. The whole journey from chapter 8 slide 1 happens again.')),
]
